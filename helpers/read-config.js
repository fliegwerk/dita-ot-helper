...
module.exports = function (configFilePath) {
    ...
    if (fs.existsSync(realPath) && fs.statSync(realPath).isFile()) {
        ...
        try {
            ...
            console.error('Could not read JSON data in config file. Please re-check the validity of your config file')
            process.exit(4)
        }
    } else {
        console.error(`Could not read config file at ${realPath}. Please check that the process has permission to read that file.`);
        process.exit(3);
    }
}
